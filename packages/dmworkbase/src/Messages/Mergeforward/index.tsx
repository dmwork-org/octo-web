...
export default class MergeforwardContent extends MessageContent {
    ...
    try {
      ...
      if (mergeForwardState.decodeDepth > MAX_MERGE_FORWARD_DEPTH) {
        ...
        if (!MergeforwardContent.depthWarningLogged) {
          console.warn(
            "[MergeforwardContent] truncating nested forwards at depth",
            mergeForwardState.decodeDepth,
            "channelType:",
  ...
  mapToMessage(messageMap: any, depth: number): Message {
    ...
    message.messageID =
      messageMap["message_id"] != null ? `${messageMap["message_id"]}` : "";
    ...
    if (
      contentType === MessageContentTypeConst.mergeForward &&
...
export class MergeforwardCell extends MessageCell<
  any,
  MergeforwardCellState
> {
  private goBackRef: React.MutableRefObject<(() => void) | null> = {
    current: null,
  };
